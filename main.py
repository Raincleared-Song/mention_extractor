...
def main_fewshot(args):
    datasets, models, config = init_all(args)
    model_d, optimizer_d = copy.deepcopy(models['model'].state_dict()), copy.deepcopy(models['optimizer'].state_dict())
    total_predict, total_instance, total_correct = 0, 0, 0
    output_file = os.path.join(config.output_path, config.model_path, 'results-fewshot-log.jsonl')
    res_writer = jsonlines.open(output_file, 'w')
    for (did, dataset) in enumerate(datasets):
        print('------', 'pair index:', did, '------')
        results = {}
        for _ in range(3):
            _, _, _, _, _, _, results = train(dataset, models, config)
            if results['correct_cnt'] > 0:
                break
    ...
    pre = total_correct / (total_predict + 1e-8)
    rec = total_correct / (total_instance + 1e-8)
